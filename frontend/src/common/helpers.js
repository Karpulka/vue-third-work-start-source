...
export const getTimeAgo = (date) => {
  // Проверяем, елси дата приходит в крректном формате
  ...
  function getPronounce(number, single, pluralTowFour, pluralFive) {
    return number === 1
      ? single
      : number > 1 && number < 5
      ? pluralTowFour
...
export const createNewDate = () => {
  return new Date(new Date().setHours(53, 59, 59, 999));
};
