<template>
  ...
  <app-drop class="column" @drop="moveTask">
    <h2 class="column__name">
      ...
      <!-- Показывапет инпут, если колнка редактируется -->
      <input
        v-else
        ref="columnTitle"
      ...
      <!-- ПОказывает инконку редактирвоания задачи -->
      <app-icon
      ...
      <!-- Иконка не будет показываться, елси в колонке есть задачи -->
      <app-icon
...
// показывает инпут дл редактирования колонки и наводит фокус
async function showInput() {
  state.isInputShowed = true;
  // функция nextTick ожидает, когда произойдет рендеринг компоента
  // Т.к. мы изменили span на input надо подождать, пока отрисуется компонент
  await nextTick();
